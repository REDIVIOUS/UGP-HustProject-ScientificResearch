...
class Gan(object):
    ...
    def __init__(self, height, width, channel, device, ngpu, ksize, z_dim, learning_rate=1e-3):
        ...
        # 构建模型
        self.encoder = Encoder(height=self.height, width=self.width, channel=self.channel, ngpu=self.ngpu, ksize=self.ksize, z_dim=self.z_dim).to(self.device)
        self.decoder = Decoder(height=self.height, width=self.width, channel=self.channel, ngpu=self.ngpu, ksize=self.ksize, z_dim=self.z_dim).to(self.device)
        self.discriminator = Discriminator(height=self.height, width=self.width, channel=self.channel, ngpu=self.ngpu, ksize=self.ksize, z_dim=self.z_dim).to(self.device)
...
class Encoder(nn.Module):
    ...
    def __init__(self, height, width, channel, ngpu, ksize, z_dim):
        ...
        self.height, self.width, self.channel, self.ngpu, self.ksize, self.z_dim = height, width, channel, ngpu, ksize, z_dim 
