...
# 搭建5层的网络，高宽层层放大，通道数层层减小
# 将长度为100的隐藏向量z调整为[b,1,1,100]的4维张量，通过转置卷积层，得到高宽为64，通道数为3的彩色图片
# 卷积层插入BN层来提高训练稳定性。
...
# 判别器类
# 接受大小为[b,64,64,3]的图片张量，连续通过5个卷积层实现特征的层层提取，输出[b,2,2,1024]
# 通过池化层转变为[b,1024]
# 全连接层获得二分类任务的概率
