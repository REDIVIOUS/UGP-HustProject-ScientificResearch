    ...
    for epoch in range(epochs):
        for step, (true_x, y) in enumerate(db_train):
            with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:
                ...
                # 计算loss
                d_loss = tf.reduce_mean(false_y) - tf.reduce_mean(true_y)
                g_loss = -tf.reduce_mean(false_y)
                # 计算gp
                t = tf.random.uniform([batch_size, 1, 1])
                t = tf.broadcast_to(t, true_x.shape)
                interplate = t * true_x + (1 - t) * false_x
                with tf.GradientTape() as tape:
                    tape.watch([interplate])
                    d_interplote_y = discriminator(interplate)
                gp_grads = tape.gradient(d_interplote_y, interplate)
                gp_grads = tf.reshape(gp_grads, [gp_grads.shape[0], -1])
                gp = tf.norm(gp_grads, axis = 1)
